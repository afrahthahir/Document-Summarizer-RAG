...
The following steps are used to ensure responses are grounded in the provided documents:

* **Smaller Chunk Size:** Documents are split into small chunks (400 characters) to improve retrieval accuracy and reduce mixing of unrelated information.
* **Cosine Similarity Retrieval:** FAISS uses cosine similarity with a threshold so only semantically relevant documents are retrieved. Irrelevant queries return no context.
* **Strict Prompting:** The LLM is instructed to answer using only the retrieved context and to respond with *“Information not found in documents”* when the context is insufficient.
* **Faithfulness Evaluation:** A faithfulness metric is used during evaluation to verify that generated answers are fully supported by the retrieved documents. The Faithfulness metric measures how factually consistent a response is with the retrieved context. It ranges from 0 to 1, with higher scores indicating better consistency.

These measures help the system avoid hallucinations and generate reliable, document-based responses.
